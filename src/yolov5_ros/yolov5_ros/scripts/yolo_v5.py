...
class Yolo_Dect:
    def __init__(self):
        ...
        self.img = rospy.Subscriber('/tutorial/image', Image, self.image_callback)
    ...
    def image_callback(self, image):
        ...
        if(self.count % 40 ==0):
            #cv2.imwrite("/home/lc/Reconstruction/src/yolov5_ros/yolov5_ros/image/" + str(self.count) + '.jpg', cv_image)
            pass
        self.count += 1
        ...
        ## color_image的shape为（480，640，3）
        self.color_image = cv2.cvtColor(self.color_image, cv2.COLOR_BGR2RGB)


        results = self.model(self.color_image)
        #rospy.loginfo(type(results))
        # results的类别是models.common.Detections

        #box的数据形式： xmin    ymin    xmax   ymax  confidence  class    name(标签)
        boxs = results.pandas().xyxy[0].values
        # rospy.loginfo(results.pandas().xyxy[0].values)
